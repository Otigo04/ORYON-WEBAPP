...
const MergedPlanes = forwardRef(({ material, width, count, height }, ref) => {
  ...
  useFrame((_, delta) => {
    ...
    // Die Perlin-Noise (cnoise) ist mit Periode 289 kachelbar. Der Zeit-Anteil
    // im Shader ist `time * uSpeed * 3 * uScale`. Wir lassen `time` exakt bei
    // einem Vielfachen dieser Periode umlaufen – dadurch ist der Loop-Übergang
    // nahtlos und der Float wächst nicht unbegrenzt (kein Präzisions-Glitch).
    const period = 289 / (uniforms.uSpeed.value * 3 * uniforms.uScale.value);
    uniforms.time.value = (uniforms.time.value + 0.1 * delta) % period;
  });
