 ...
 * Funktionen faden passende Sections sanft ein (Hero, Portfolio, Leistungen,
...
function tokensFor(dark: boolean): Tokens {
  return dark
    ...
    : {
        dark,
        // Hell-Modus: bewusst kräftig & kontrastreich, damit klar als cleane,
        // weiße Website erkennbar ist (dunkle Schrift auf reinem Weiß).
        accent: "#047857", // emerald-700, ruhig & hochwertig auf Weiß
        accentText: "#ffffff",
        accentSoft: "rgba(4,120,87,0.12)",
        surface: "bg-white",
        surfaceAlt: "bg-[#f1f3f2]",
        text: "text-neutral-700",
        heading: "text-neutral-950",
        muted: "text-neutral-500",
        card: "bg-white border-neutral-200 shadow-[0_1px_3px_rgba(0,0,0,0.06)]",
        border: "border-neutral-200",
        block: "bg-neutral-300",
      };
}
...
export function LivePreview({ data }: { data: BriefData }) {
  ...
  return (
    <div className="relative overflow-hidden rounded-3xl border border-white/10 bg-black/40 shadow-[0_20px_60px_-25px_rgba(0,0,0,0.95)]">
      ...
      {/* Gerenderte „Seite" */}
      <div className={`relative max-h-[720px] overflow-y-auto ${t.surface} transition-colors duration-300`}>
        {/* Navbar */}
        <div className={`sticky top-0 z-10 flex items-center justify-between border-b ${t.border} ${t.surface} px-4 py-2.5`}>
...
function Bar({ w, t, strong }: { w: string; t: Tokens; strong?: boolean }) {
  return (
    <span
      className={`block h-2 rounded-full ${strong ? (t.dark ? "bg-white/25" : "bg-neutral-400") : t.dark ? "bg-white/10" : "bg-neutral-200"}`}
      style={{ width: w }}
    />
  );
}
